 ...
 * os_sched.h
 *
 * \brief   Get the time in microseconds since os_sched_start() API function was called.
 *
...
extern bool (*os_sched_in_restore)(void);

void os_delay_rom(uint32_t ms);
uint32_t os_sys_time_get_rom(void);
bool os_sched_start_rom(void);
bool os_sched_stop_rom(void);
bool os_sched_suspend_rom(void);
