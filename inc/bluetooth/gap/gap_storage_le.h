 ...
 * \code{.c}
    void test(void)
    {
        ...
        T_CCCD_DATA *p_cccd_data = os_mem_zalloc(RAM_TYPE_DATA_ON, 4 + ccc_bits_count * 4);
...
bool le_gen_bond_dev(uint8_t *bd_addr, T_GAP_REMOTE_ADDR_TYPE bd_type,
                     ...
                     uint8_t ltk_length, uint8_t *p_ltk, T_LE_KEY_TYPE key_type, T_CCCD_DATA *p_cccd);
 ...
 *
 * @param[in] remote_bd        Bluetooth device address.
 * @return Operation result.
 * @retval true     Operation success.
 * @retval false    Operation failure.
 */
bool le_delete_dev_info(T_LE_REMOTE_BD remote_bd);

/**
 * @brief   Get device priority information.
 *
 * @param[out] p_bond_num        Pointer to the bonded device number to read.
 * @param[out] p_pri_table        Pointer to priority information to read.
 * @return Operation result.
 * @retval true     Operation success.
 * @retval false    Operation failure.
 */
