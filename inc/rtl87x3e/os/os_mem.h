...
#define os_mem_zalloc2(size)   \
    os_mem_zalloc_intern(RAM_TYPE_ITCM1, size, __func__, __LINE__)
...
#define os_mem_alloc2(size)    \
    os_mem_alloc_intern(RAM_TYPE_ITCM1, size, __func__, __LINE__)
...
void *os_mem_alloc_intern_rom(RAM_TYPE ram_type, size_t size,
                              const char *p_func, uint32_t file_line);
void *os_mem_zalloc_intern_rom(RAM_TYPE ram_type, size_t size,
                               const char *p_func, uint32_t file_line);
void *os_mem_aligned_alloc_intern_rom(RAM_TYPE ram_type, size_t size, uint8_t alignment,
                                      const char *p_func, uint32_t file_line);
