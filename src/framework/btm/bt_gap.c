...
void bt_handle_sdp_discov_info(T_GAP_SDP_ATTR_INFO *p_info,
                               T_BT_SDP_ATTR_INFO  *p_sdp)
{
    ...
    p_sdp->p_attr = p_info->p_attr;
    p_sdp->attr_len = p_info->attr_len;
    ...
    {
        if (gap_br_get_sdp_uuid_value(p_elem, p_attr_end, (T_GAP_UUID_TYPE *)(&p_sdp->srv_class_uuid_type),
                                      (T_GAP_UUID_DATA *)(&p_sdp->srv_class_uuid_data)))
        {
            break;
        }
        else
    ...
    if (p_attr)
    {
        ...
        while ((p_elem = gap_br_access_sdp_elem(p_attr, p_attr_end, loop)) != NULL)
        {
            ...
            if (p_attr_param)
            {
                p_sdp->profile_version = (uint16_t)gap_br_get_sdp_value(p_attr_param, p_attr_end);
                break;
            }
            loop++;
        }
    ...
    p_attr = gap_br_find_sdp_attr(p_attr_start, p_attr_end, SDP_ATTR_PROTO_DESC_LIST);
    loop = 1;
    while ((p_elem = gap_br_access_sdp_elem(p_attr, p_attr_end, loop)) != NULL)
    {
        p_attr_param = gap_br_access_sdp_elem(p_elem, p_attr_end, 2);
        ...
        if (p_attr_param)
        {
            switch ((uint16_t)gap_br_get_sdp_value(gap_br_access_sdp_elem(p_elem, p_attr_end, 1), p_attr_end))
            {
            case UUID_RFCOMM:
                p_sdp->server_channel = (uint8_t)gap_br_get_sdp_value(p_attr_param, p_attr_end);
                break;

            default:
                p_sdp->protocol_version = (uint16_t)gap_br_get_sdp_value(p_attr_param, p_attr_end);
                break;
            }
        }
        loop++;
    ...
    if (p_attr)
    {
        p_sdp->supported_feat = (uint16_t)gap_br_get_sdp_value(p_attr, p_attr_end);
    }

    /* get l2cap psm */
    p_attr = gap_br_find_sdp_attr(p_attr_start, p_attr_end, SDP_ATTR_L2C_PSM);
    if (p_attr)
    {
        p_sdp->l2c_psm = (uint16_t)gap_br_get_sdp_value(p_attr, p_attr_end);
    }

    /* get supported repositories */
    p_attr = gap_br_find_sdp_attr(p_attr_start, p_attr_end, SDP_ATTR_SUPPORTED_REPOSITORIES);
    if (p_attr)
    {
        p_sdp->supported_repos = (uint8_t)gap_br_get_sdp_value(p_attr, p_attr_end);
    }

    /* for pbap, supported features is 0x0317 and size is uint32_t while 10 bits are used now */
    p_attr = gap_br_find_sdp_attr(p_attr_start, p_attr_end, SDP_ATTR_PBAP_SUPPORTED_FEAT);
    if (p_attr)
    {
        p_sdp->pbap_supported_feat = gap_br_get_sdp_value(p_attr, p_attr_end);
    }
    else
    {
        p_sdp->pbap_supported_feat = 0x00000003;
    }
}

void bt_handle_acl_status_info(T_GAP_ACL_STATUS_INFO *p_info)
{
    T_BT_BR_LINK *p_link;
    ...
    BTM_PRINT_INFO3("bt_handle_acl_status_info: bd_addr %s, link %p, status 0x%02x",
                    TRACE_BDADDR(p_info->bd_addr), p_link, p_info->status);
    ...
    case GAP_ACL_CONN_ACTIVE:
        if (p_link != NULL)
        {
            bt_pm_sm(p_link, BT_PM_EVENT_SNIFF_EXIT_SUCCESS);
    ...
    case GAP_ACL_CONN_SNIFF:
        if (p_link != NULL)
        {
            uint16_t  interval = p_info->p.sniff.interval;

            bt_pm_sm(p_link, BT_PM_EVENT_SNIFF_ENTER_SUCCESS);
    ...
    case GAP_ACL_CONN_NOT_ENCRYPTED:
        if (p_link != NULL)
        {
            uint16_t  cause = p_info->p.authen.cause;
    ...
    case GAP_ACL_CONN_BR_SC_START:
        if (p_link != NULL)
        {
            p_link->acl_link_sc_ongoing = true;
    ...
    case GAP_ACL_CONN_BR_SC_CMPL:
        if (p_link != NULL)
        {
            uint16_t  cause = p_info->p.br_sc_cmpl.cause;
            p_link->acl_link_sc_ongoing = false;
    ...
    case GAP_ACL_CONN_SUCCESS:
        {
            ...
            if (p_link != NULL)
            {
                BTM_PRINT_INFO2("acl (%x) link connect success, handle 0x%x",
                                p_link->link_id, p_info->p.conn_success.handle);

                uint16_t  handle = p_info->p.conn_success.handle;
                p_link->acl_link_state = BT_LINK_STATE_CONNECTED;
                p_link->acl_handle = handle;

                payload.msg_buf = &handle;

                remote_peer_addr_get(peer_addr);

                if (memcmp(p_info->bd_addr, peer_addr, 6) == 0)
                {
                ...
                {
                    interval = BT_SNIFF_INTERVAL_BASE + p_link->link_id * BT_SNIFF_INTERVAL_OFFSET;
                    timeout = BT_PM_TIMEOUT;
                }

                bt_sniff_mode_config(p_info->bd_addr, interval, BT_SNIFF_ATTEMPT, BT_SNIFF_TIMEOUT, timeout);
    ...
    case GAP_ACL_CONN_READY:
        if (p_link != NULL)
        {
            bt_pm_sm(p_link, BT_PM_EVENT_LINK_CONNECTED);
    ...
    case GAP_ACL_CONN_FAIL:
        {
            uint16_t  cause = p_info->p.conn_fail.cause;

            BTM_PRINT_ERROR2("acl connection fail to bd_addr [%s] with reason: 0x%02X",
                             TRACE_BDADDR(p_info->bd_addr), cause);

            payload.msg_buf = &cause;

            if (p_link != NULL)
            {
                bt_free_br_link(p_link);
            }
    ...
    case GAP_ACL_CONN_DISCONN:
        if (p_link != NULL)
        {
            uint16_t  cause = p_info->p.conn_disconn.cause;

            BTM_PRINT_TRACE2("acl (%x) link disconnected, status=0x%02X",
                             p_link->link_id, p_info->p.conn_disconn.cause);

            if (p_link->sco_handle != 0)
            {
                payload.msg_buf = &p_info->p.conn_disconn.cause;
    ...
    case GAP_ACL_ROLE_MASTER:
        if (p_link != NULL)
        {
            ...
            p_link->role_switch_status = BT_ROLE_SWITCH_IDLE;
    ...
    case GAP_ACL_AUTHEN_FAIL:
        if (p_link != NULL)
        {
            ...
            p_link->acl_link_authenticated = false;
    ...
    case GAP_ACL_AUTHEN_SUCCESS:
        if (p_link != NULL)
        {
            p_link->acl_link_authenticated = true;
                  ...
                  T_GAP_BR_MSG  br_msg)
{
    ...
    case GAP_BR_SDP_ATTR_INFO:
        {
            ...
            bt_handle_sdp_discov_info(p, &sdp_attr_info);
    ...
    case GAP_BR_SCO_CONN_IND:
        {
            ...
            p_link = bt_find_br_link(p_data->bd_addr);
            if (p_link != NULL)
            {
                p_link->is_esco = p_data->is_esco;
    ...
    case GAP_BR_SCO_CONN_CMPL:
        {
            ...
            if (p_link != NULL)
            {
                ...
                {
                    p_link->is_esco = p_data->is_esco;
                    p_link->sco_handle = p_data->handle;
                    p_link->sco_air_mode = p_data->air_mode;
                    if (p_data->is_esco)
                    {
                        p_link->sco_packet_length = p_data->rx_length;
                    }
                    else
                    {
                        p_link->sco_packet_length = 30;
                    }
                }
    ...
    case GAP_BR_SCO_DATA_IND:
        {
            ...
            if (p_link != NULL)
            {
                ...
                sco_data_ind.handle = p_link->sco_handle;
                ...
                if (p_link->sco_air_mode == GAP_AIR_MODE_TRANSPARENT)
                {
                        ...
                        while (i < sco_len)
                        {
                            ...
                            case 0:
                                if (bt_sco_msbc_header_check(&sco_buf[i], sco_len - i) == true)
                                {
                                    if (sco_len - i >= 3)
                                    {
                                        memcpy(p_link->sco_buf, &sco_buf[i], 3);
                                        p_link->curr_sco_len = 3;
                                        i += 3;
                                    }
                                    else
                                    {
                                        memcpy(p_link->sco_buf, &sco_buf[i], sco_len - i);
                                        p_link->curr_sco_len = sco_len - i;
                                        i += sco_len - i;
                                    }
                                }
                                else
                            ...
                            case 1:
                                if (sco_len - i >= 2)
                                {
                                    p_link->sco_buf[1] = sco_buf[i];
                                    p_link->sco_buf[2] = sco_buf[i + 1];

                                    if (bt_sco_msbc_header_check(p_link->sco_buf, 3) == true)
                                    {
                                        p_link->curr_sco_len = 3;
                                        i += 2;
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (bt_sco_msbc_header_check(p_link->sco_buf, 2) == true)
                                    {
                                        p_link->curr_sco_len = 2;
                                        i++;
                                    }
                                    else
                                    {
                                        p_link->curr_sco_len = 0;
                                        memset(p_link->sco_buf, 0, 2);
                                    }
                                }
                                break;
                            ...
                            default:
                                if (SCO_PACKET_LEN - p_link->curr_sco_len <= sco_len - i)
                                {
                                    memcpy(&p_link->sco_buf[p_link->curr_sco_len], &sco_buf[i],
                                           SCO_PACKET_LEN - p_link->curr_sco_len);
                                    sco_data_ind.length = SCO_PACKET_LEN - 3;
                                    sco_data_ind.p_data = &p_link->sco_buf[2];
                                    i += SCO_PACKET_LEN - p_link->curr_sco_len;
                                    p_link->curr_sco_len = 0;
                                    bt_mgr_dispatch(BT_MSG_SCO_DATA_IND, &payload);
                                }
                                else
                                {
                                    memcpy(&p_link->sco_buf[p_link->curr_sco_len], &sco_buf[i], sco_len - i);
                                    p_link->curr_sco_len += sco_len - i;
                                    i += sco_len - i;
                                }
                                break;
                    ...
                    {
                        p_link->curr_sco_len = 0;
                        memset(p_link->sco_buf, 0, SCO_PACKET_LEN);
                ...
                else /* CVSD */
                {
                    ...
                    {
                        memset(p_link->sco_buf, 0x55, SCO_PACKET_LEN);
                        sco_data_ind.length = sco_len;
                        sco_data_ind.p_data = p_link->sco_buf;
                    }
    ...
    case GAP_BR_ACL_CONN_IND:
        {
            ...
            p_link = bt_find_br_link(p_ind->bd_addr);
            if (p_link)
            {
            ...
            p_link = bt_alloc_br_link(p_ind->bd_addr);
            if (p_link == NULL)
            {
    ...
    case GAP_BR_LINK_KEY_INFO:
        {
            T_GAP_LINK_KEY_INFO *p_info = (T_GAP_LINK_KEY_INFO *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_LINK_KEY_REQ_IND:
        {
            T_GAP_LINK_KEY_REQ_IND *p_info = (T_GAP_LINK_KEY_REQ_IND *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_PIN_CODE_REQ_IND:
        {
            T_GAP_PIN_CODE_REQ_IND *p_info = (T_GAP_PIN_CODE_REQ_IND *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_USER_CONFIRM_REQ:
        {
            T_GAP_USER_CFM_REQ_IND *p_info = (T_GAP_USER_CFM_REQ_IND *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_PASSKEY_INPUT:
        {
            T_GAP_USER_PASSKEY_REQ_IND *p_info = (T_GAP_USER_PASSKEY_REQ_IND *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_PASSKEY_DISPLAY:
        {
            T_GAP_USER_PASSKEY_NOTIF_INFO *p_info = (T_GAP_USER_PASSKEY_NOTIF_INFO *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_READ_RSSI_RSP:
        {
            T_GAP_READ_RSSI_RSP *p_info = (T_GAP_READ_RSSI_RSP *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
    ...
    case GAP_BR_SETUP_QOS_RSP:
        {
            ...
            p_link = bt_find_br_link_by_handle(p_rsp->handle);
            if (p_link)
            {
                memcpy(payload.bd_addr, p_link->bd_addr, 6);
                payload.msg_buf = p_buf;
    ...
    case GAP_BR_SETUP_QOS_CMPL:
        {
            T_GAP_SETUP_QOS_CMPL *p_info = (T_GAP_SETUP_QOS_CMPL *)p_buf;
            memcpy(payload.bd_addr, p_info->bd_addr, 6);
            payload.msg_buf = p_buf;
