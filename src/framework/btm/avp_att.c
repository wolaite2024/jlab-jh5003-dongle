...
typedef bool(*P_FUN_AVP_ATT_GENERAL_CB)(uint8_t service_id, uint16_t cid, uint8_t *data,
                                        uint16_t length);

static P_FUN_AVP_ATT_GENERAL_CB pfn_simp_ext_service_cb = NULL;

static uint8_t avp_att_read_value[] = {0x01, 0x01, 0x00, 0x04, 0x07, 0x02, 0x01, 0x01};
                                           ...
                                           uint16_t attrib_index, uint16_t offset, uint16_t *p_length, uint8_t **pp_value)
{
    ...
    switch (attrib_index)
    {
    case AVP_ATT_SERVICE_CHAR_READ_INDEX:
        {
            *pp_value = avp_att_read_value;
            *p_length = sizeof(avp_att_read_value);
        }
        break;

    // case AVP_ATT_SERVICE_CHAR_READ_INDEX0:
    // case AVP_ATT_SERVICE_CHAR_READ_INDEX1:
    // case AVP_ATT_SERVICE_CHAR_READ_INDEX2:
    case AVP_ATT_SERVICE_CHAR_READ_INDEX3:
        {
                                           ...
                                           P_FUN_EXT_WRITE_IND_POST_PROC *p_write_post_proc)
{
    ...
    *p_write_post_proc = avp_att_write_post_callback;
    ...
    {
        pfn_simp_ext_service_cb((uint8_t)service_id, cid, p_value, length);
    }
