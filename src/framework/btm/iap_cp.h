...
#define CP_AUTH_VER         0x01    //RO, authentication verision(just for cp v3.0)
...
#define CP_SIG_LEN          0x11    //RW, signature data length, 2bytes(cp v2.0 128, cp v3.0 64)
#define CP_SIG_DATA         0x12    //RW, signature data, 128 bytes for cp v2.0, 64 bytes for cp v3.0
#define CP_REG_CHA_LEN      0x20    //RW, challenge data length, 2bytes(cp v2.0 20, cp v3.0 32)
#define CP_REG_CHA_DATA     0x21    //RW, challenge data, 128 bytes for cp v2.0, 32 bytes for cp v3.0

//cp register address (block 3)
#define CP_REG_ACD_LEN      0x30    //RO, accessory certificate data length, 2bytes(cp v2.0 <=1280, cp v3.0 607~609)
...
//cp register address (block 6)
#define CP_REG_SLEEP_MODE        0x60    //RW, force control cp enter sleep mode
...
typedef enum
{
    ...
    CP_PRO_RES_CER_VAL_OK   = 0x40      //!<pple device certificate successfully validated
} T_CP_PRO_RES;
...
typedef enum
{
    ...
    CP_CMD_START_CHA_GEN  = 0x02,    //!<start challenge generate process
    CP_CMD_START_SIG_VEFY = 0x03,    //!<start signature-verification process
    CP_CMD_START_CER_VAL  = 0x04,    //!<start certificate-validation process
} T_CP_CMD;
...
typedef enum
{
    ...
    CP_ERR_INV_READ_REG   = 0x01,       //!<Invalid register for read
    CP_ERR_INV_WR_REG     = 0x02,       //!<Invalid register for write
    CP_ERR_INV_SIG_LEN    = 0x03,       //!<Invalid signature length
    CP_ERR_INV_CHA_LEN    = 0x04,       //!<Invalid challenge length
    CP_ERR_INV_CER_LEN    = 0x05,       //!<Invalid certificate length
    CP_ERR_INTR_SIG_GEN   = 0x06,       //!<Internal process error during signature generation
    CP_ERR_INTR_CHA_GEN   = 0x07,       //!<Internal process error during challenge generation
    CP_ERR_INTR_SIG_VEFY  = 0x08,       //!<Internal process error during signature verification
    CP_ERR_INTR_VER_VAL   = 0x09,       //!<Internal process error during certificate validation
    CP_ERR_INV_CTRL       = 0x0a,       //!<Invalid process control
    CP_ERR_PRO_OUT_OF_SEQ = 0x0b,       //!<Process control out of sequence
    CP_ERR_RESVD          = 0xff,
} T_CP_ERROR_CODE;
...
/**
    * @brief  CP init
    * @param  p_cp_write        to set the cp write func
    * @param  p_cp_read   to set the cp burst read func
    * @return none
    */
