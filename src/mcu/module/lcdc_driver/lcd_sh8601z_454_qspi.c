...
static void spic3_spi_write(uint8_t *buf, uint32_t len)
{
    ...
    fmc_spic2_set_user_addr_len(3);

    if (len > 4)
    {
        fmc_spic2_set_txndf(len - 4);
    }
    else
    {
    ...
    {
        fmc_spic2_set_dr(buf[i], FMC_SPIC_DATA_BYTE);
    }
...
static void SH8601A_Reg_Write(const SH8601A_CMD_DESC *cmd)
{
    ...
    while (cmd[idx].instruction != SH8601A_QSPI_SEQ_FINISH_CODE)
    {
        ...
        spic3_spi_write(sdat, cmd[idx].wordcount + 4);
        if (cmd[idx].delay != 0)
        {
...
static void rtl_SH8601A_qspi_enter_data_output_mode(uint32_t len_byte)
{
    ...
    fmc_spic2_set_multi_ch(FMC_SPIC_SING_CH, FMC_SPIC_SING_CH, FMC_SPIC_QUAD_CH);
//    fmc_spic2_set_multi_ch(FMC_SPIC_SING_CH, FMC_SPIC_QUAD_CH, FMC_SPIC_QUAD_CH);
    fmc_spic2_set_tx_mode();
...
//    uint32_t first_word = 0x12 | __REV(0x002c00);
    fmc_spic2_set_dr(first_word, FMC_SPIC_DATA_WORD);
