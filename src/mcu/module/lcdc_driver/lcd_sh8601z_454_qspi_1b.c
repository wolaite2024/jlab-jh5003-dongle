...
*/
#include "app_gui.h"
#if (TARGET_LCD_DEVICE == LCD_DEVICE_SH8601Z_QSPI_1_BIT)
...
static void spic3_spi_write_command(uint8_t cmd)
{
    lcd_dcx_reset();
    ...
    fmc_spic2_set_txndf(0);

    fmc_spic2_set_dr(cmd, FMC_SPIC_DATA_BYTE);
...
static void spic3_spi_write_data(uint8_t *data, uint32_t len)
{
    lcd_dcx_set();
    fmc_spic2_enable(false);
    fmc_spic2_set_user_mode();
    ...
    fmc_spic2_set_user_addr_len(0);
    fmc_spic2_set_txndf(len);

    for (uint32_t i = 0; i < len; i++)
    {
        fmc_spic2_set_dr(data[i], FMC_SPIC_DATA_BYTE);
    }
...
{
    lcd_cs_pin_active();
    spic3_spi_write_command(buf[0]);

    if (len > 1)
    {
        spic3_spi_write_data(&buf[1], len - 1);
    }
...
static const SH8601A_CMD_DESC SH8601A_POST_OTP_POWERON_SEQ_CMD[] =
{
    ...
    {SH8601A_QSPI_INST_CMD_WRITE, 0x44, 1,  2,  {0x00, 0x00}},
    {SH8601A_QSPI_INST_CMD_WRITE, 0x35, 1,  1,  {0x00}},
#else
    {SH8601A_QSPI_INST_CMD_WRITE, 0x34, 1,  0,  {0x00}},
#endif
    ...
    {SH8601A_QSPI_INST_CMD_WRITE, 0x3A, 1,  1,  {0x55}},
...
static void SH8601A_Reg_Write(const SH8601A_CMD_DESC *cmd)
{
    ...
    uint8_t *sdat = os_mem_alloc(RAM_TYPE_DATA_ON, 512);
    while (cmd[idx].instruction != SH8601A_QSPI_SEQ_FINISH_CODE)
    {
        ...
        spic3_spi_write(sdat, cmd[idx].wordcount + 1);
        if (cmd[idx].delay != 0)
        {
...
static void rtl_SH8601A_qspi_enter_data_output_mode(uint32_t len_byte)
{
    lcd_cs_pin_active();
    spic3_spi_write_command(0x2c);
    ...
    if (len_byte > 0)
    {
        ...
        fmc_spic2_set_cmd_len(0);
        fmc_spic2_set_user_addr_len(0);
        fmc_spic2_set_txndf(len_byte);

        SPIC2->DMACR = 0x02;
        SPIC2->DMATDLR = 4;

        uint32_t ctrlr0 = SPIC2->CTRLR0;
        ctrlr0 = ctrlr0 | BIT(11);
        SPIC2->CTRLR0 = ctrlr0;
...
void lcd_SH8601A_qspi_454_set_window(uint16_t xStart, uint16_t yStart, uint16_t xEnd, uint16_t yEnd)
{
    uint8_t data[4];
    data[0] = xStart >> 8;
    data[1] = xStart & 0xff;
