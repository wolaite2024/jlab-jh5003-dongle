...
void GDMA_Init(GDMA_ChannelTypeDef *GDMA_Channelx, GDMA_InitTypeDef *GDMA_InitStruct)
{
    ...
    if (GDMA_InitStruct->GDMA_ChannelNum < 8)
    {
    ...
    {
        /* mask pending interrupts of corresponding GDMA channel */
        temp_bit = BIT(GDMA_InitStruct->GDMA_ChannelNum + 16);
        GDMA_BASE->MASK_TFR = temp_bit;
        GDMA_BASE->MASK_BLOCK = temp_bit;
        GDMA_BASE->MASK_DST_TRAN = temp_bit;
        GDMA_BASE->MASK_SRC_TRAN = temp_bit;
    ...
    if (GDMA_InitStruct->GDMA_Gather_En == 1 &&
        ((GDMA_Channelx == GDMA_Channel2) || (GDMA_Channelx == GDMA_Channel3) ||
         (GDMA_Channelx == GDMA_Channel4) || (GDMA_Channelx == GDMA_Channel5))) //GDMA_GatherCount
    {
        GDMA_Channelx->CTL_LOW |= BIT(17);
        GDMA_Channelx->SGR_LOW = GDMA_InitStruct->GDMA_GatherCount << 20 |
                                 GDMA_InitStruct->GDMA_GatherInterval;
        GDMA_Channelx->SGR_HIGH = GDMA_InitStruct->GDMA_Source_Cir_Gather_Num;
    }
         ...
         (GDMA_Channelx == GDMA_Channel4) || (GDMA_Channelx == GDMA_Channel5)))
    {
        ...
        GDMA_Channelx->DSR_LOW = GDMA_InitStruct-> GDMA_ScatterCount << 20 |
                                 GDMA_InitStruct->GDMA_ScatterInterval;
        GDMA_Channelx->DSR_HIGH = GDMA_InitStruct->GDMA_Dest_Cir_Sca_Num;
    }
    ...
    if (GDMA_InitStruct->GDMA_DestHandshake >= 24 && GDMA_InitStruct->GDMA_DestHandshake <= 31)
    {
...
void GDMA_StructInit(GDMA_InitTypeDef *GDMA_InitStruct)
{
    ...
    GDMA_InitStruct->GDMA_Source_Cir_Gather_Num = 1;
    GDMA_InitStruct->GDMA_Dest_Cir_Sca_Num = 1;
...
{
    uint32_t  GDMA_ChannelNum_ADJ;
    GDMA_ChannelTypeDef *gdma_reg;
    ...
    gdma_reg = (GDMA_ChannelTypeDef *)DMA_CH_BASE(GDMA_ChannelNum);
    GDMA_ChannelNum_ADJ = ((GDMA_ChannelNum >= 8) ? (GDMA_ChannelNum + 8) : (GDMA_ChannelNum));

    if (NewState != DISABLE)
    {
        /* Enable the selected DMAy Channelx */
        GDMA_BASE->ChEnReg |= BIT(GDMA_ChannelNum_ADJ) | BIT(GDMA_ChannelNum_ADJ + 8);
        return GDMA_ENABLE_SUCCESS;
    }
    else
    {
        /*gdma transfor not finished */
        if (GDMA_BASE->ChEnReg & BIT(GDMA_ChannelNum_ADJ))
        {
            /* suspend gdma channel */
            gdma_reg->CFG_LOW |= GDMA_SUSPEND_TRANSMISSSION;
            /* Disable the selected DMAy Channelx */
            /* polling fifo empty */
            while ((GDMA_GetSuspendChannelStatus(gdma_reg) != SET) && --timeout);
            /*cfg bit0 not set */
            if (GDMA_GetSuspendChannelStatus(gdma_reg) != SET)
            {
                timeout = bit_need_check_times;
                /*polling cfg[1:2] 10 times set in 100timer check */
                while (--timeout)
                {
                    if ((gdma_reg->CFG_LOW & BIT1) && (gdma_reg->CFG_LOW & BIT2))
                    {
                        bit_set_time++;
            ...
            /* disable dma */
            GDMA_BASE->ChEnReg = BIT(GDMA_ChannelNum_ADJ + 8);
...
void GDMA_INTConfig(uint8_t GDMA_ChannelNum, uint32_t GDMA_IT, FunctionalState NewState)
{
    ...
    if (NewState != DISABLE)
    {
        ...
        {
            /* Enable the selected DMA interrupts */
            temp_bit = BIT(GDMA_ChannelNum + 8) | BIT(GDMA_ChannelNum + 16);
    ...
    else
    {
        ...
        {
            /* Disable the selected DMA interrupts */
            temp_bit = BIT(GDMA_ChannelNum  + 16);
        }
        if (GDMA_IT & GDMA_INT_Transfer)
        {
            GDMA_BASE->MASK_TFR = temp_bit;
...
bool GDMA_SafeSuspend(GDMA_ChannelTypeDef *GDMA_Channelx)
{
    ...
    if ((GDMA_Channelx->CFG_LOW & GDMA_SUSPEND_CHANNEL_STATUS) != SET)
    {
        timeout = bit_need_check_times;
        /*polling cfg[1:2] 10 times set in 100timer check */
        while (--timeout)
