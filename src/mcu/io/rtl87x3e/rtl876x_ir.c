...
void IR_Init(IR_InitTypeDef *IR_InitStruct)
{
    ...
    IR->CLK_DIV = (IR_InitStruct->IR_Clock) / (IR_InitStruct->IR_Freq * 1000) - 1;
    ...
    if (IR_InitStruct->IR_Mode == IR_MODE_TX)
    {
        ...
        IR->TX_CONFIG = (IR_InitStruct->IR_Mode) | \
                        ...
                        (((uint32_t)((IR->CLK_DIV + 1) / (IR_InitStruct->IR_DutyCycle) - 1)) << IR_TX_DUTY_NUM_Pos);
...
{
    IR_InitStruct->IR_Clock             = 40000000;
