...
{
#if (F_BT_LE_5_1_CLIENT_SUPPORTED_FEATURES_CLIENT_SUPPORT || F_BT_LE_5_1_GATT_CACHING_CLIENT_SUPPORT)
    bool hdl_valid = false;
    bool read_using_char_uuid = false;
    uint16_t  handle;
    uint16_t  start_handle = 0x0001;
    uint16_t  end_handle = 0xFFFF;
    uint16_t uuid16 = 0;
    uint8_t *p_uuid128 = NULL;
    uint16_t *hdl_cache;

    if (conn_id >= gatts_link_num)
    {
        PROFILE_PRINT_ERROR1("gatts_read: failed invalid conn_id %d", conn_id);
        return false;
    }
    hdl_cache = gatts_table[conn_id].hdl_cache;

    PROFILE_PRINT_TRACE2("gatts_read: conn_id %d, read_type %d", conn_id, read_type);

    switch (read_type)
    {
#if F_BT_LE_5_1_CLIENT_SUPPORTED_FEATURES_CLIENT_SUPPORT
    case GATTS_READ_CLIENT_SUPPORTED_FEATURES:
        if (hdl_cache[HDL_GATTS_CLIENT_SUPPORTED_FEATURES])
        {
            hdl_valid = true;
            handle = hdl_cache[HDL_GATTS_CLIENT_SUPPORTED_FEATURES];
        }
        else
        {
            read_using_char_uuid = true;
            uuid16 = GATT_UUID_CHAR_CLIENT_SUPPORTED_FEATURES;
        }
        break;
#endif

#if F_BT_LE_5_1_GATT_CACHING_CLIENT_SUPPORT
    case GATTS_READ_DATABASE_HASH:
        read_using_char_uuid = true;
        uuid16 = GATT_UUID_CHAR_DATABASE_HASH;
        break;
#endif
    ...
    if (read_using_char_uuid)
    {
        if (client_attr_read_using_uuid(conn_id, gatts_client, start_handle, end_handle, uuid16,
                                        p_uuid128) == GAP_CAUSE_SUCCESS)
        {
            return true;
        }
    }
    else
    {
        if (hdl_valid)
        {
            if (client_attr_read(conn_id, gatts_client, handle) == GAP_CAUSE_SUCCESS)
            {
                return true;
            }
        }
...
#endif
    PROFILE_PRINT_WARN0("gatts_read: Request fail! Please check!");
    return false;
}
                                                ...
                                                uint8_t *p_client_supp_feats)
{
    ...
    if (conn_id >= gatts_link_num)
    {
        PROFILE_PRINT_ERROR1("gatts_client_set_client_supported_features: failed invalid conn_id %d",
                             conn_id);
        return false;
    }

    handle = gatts_table[conn_id].hdl_cache[HDL_GATTS_CLIENT_SUPPORTED_FEATURES];

    if (handle)
    {
        if (client_attr_write(conn_id, gatts_client, GATT_WRITE_TYPE_REQ, handle,
                              len, p_client_supp_feats) == GAP_CAUSE_SUCCESS)
        {
            return true;
        }
    }

    APP_PRINT_WARN0("gatts_client_set_client_supported_features: Request fail! Please check!");
    return false;
}
#endif
...
#if F_BT_LE_5_1_CLIENT_SUPPORTED_FEATURES_CLIENT_SUPPORT
            case GATT_UUID_CHAR_CLIENT_SUPPORTED_FEATURES:
                gatts_table[conn_id].hdl_cache[HDL_GATTS_CLIENT_SUPPORTED_FEATURES] = handle;
                break;
#endif

#if F_BT_LE_5_1_GATT_CACHING_CLIENT_SUPPORT
            case GATT_UUID_CHAR_DATABASE_HASH:
                gatts_table[conn_id].hdl_cache[HDL_GATTS_DATABASE_HASH] = handle;
                break;
#endif
...
static void gatts_client_read_result_cb(uint8_t conn_id,  uint16_t cause,
                                        uint16_t handle, uint16_t value_size, uint8_t *p_value)
{
    ...
    uint16_t *hdl_cache;
    hdl_cache = gatts_table[conn_id].hdl_cache;
    cb_data.cb_type = GATTS_CLIENT_CB_TYPE_READ_RESULT;
    cb_data.cb_content.read_result.cause = cause;

    PROFILE_PRINT_INFO2("gatts_client_read_result_cb: handle 0x%x, cause 0x%x", handle, cause);

#if F_BT_LE_5_1_CLIENT_SUPPORTED_FEATURES_CLIENT_SUPPORT
    if (handle == hdl_cache[HDL_GATTS_CLIENT_SUPPORTED_FEATURES])
    {
        hdl_valid = true;
        cb_data.cb_content.read_result.type = GATTS_READ_CLIENT_SUPPORTED_FEATURES;
        if (cause == GAP_SUCCESS)
        {
            cb_data.cb_content.read_result.data.client_supported_features.value_size = value_size;
            cb_data.cb_content.read_result.data.client_supported_features.p_value = p_value;
        }
    }
#endif
#if F_BT_LE_5_1_GATT_CACHING_CLIENT_SUPPORT
    /* TBD */
    if ((handle == hdl_cache[HDL_GATTS_DATABASE_HASH]) || ((!hdl_valid) &&
                                                           (value_size == GATTS_DATABASE_HASH_LEN)))
    {
        hdl_valid = true;
        cb_data.cb_content.read_result.type = GATTS_READ_DATABASE_HASH;
        if (cause == GAP_SUCCESS)
        {
            memcpy(cb_data.cb_content.read_result.data.database_hash, p_value, GATTS_DATABASE_HASH_LEN);
        }
    }
#endif

    /* Inform application the read result. */
    if (hdl_valid && gatts_client_cb)
    {
        (*gatts_client_cb)(gatts_client, conn_id, &cb_data);
    }
#endif
...
const T_FUN_CLIENT_CBS gatts_client_cbs =
{
    ...
    gatts_client_read_result_cb,      //!< Read response callback function pointer
