        ...
        if (type == GATT_MSG_GAPS_CLIENT_READ_RESULT)
        {
            ...
            if (p_read->cause == APP_RESULT_SUCCESS)
            {
                ...
                case GATT_UUID_CHAR_DEVICE_NAME:
                    {
                        APP_PRINT_INFO2("GATT_MSG_GAPS_CLIENT_READ_RESULT: device_name_length %d, device_name %b",
                                        p_read->value_len, TRACE_BINARY(p_read->value_len, p_read->data.p_device_name));
                    }
                    break;
