 ...
 *                              Macros
 *============================================================================*/
/** @defgroup IR_NEC_Protocol_Exported_Functions IR NEC Protocol Exported Functions
  * @brief
  * @{
  */
...
/** @} */ /* End of group IR_NEC_Protocol_Exported_Functions */
...
* @ LSB is sent first !
*/
static const IR_ProtocolTypeDef NEC_PROTOCOL =
{
    38,                                                 /* Carrier freqency */
    2,                                                  /* headerLen */
    {PULSE_HIGH | 9000, PULSE_LOW | (4500 - 26)},              /* headerBuf */
    {PULSE_HIGH | 560, PULSE_LOW | (560 - 26)},                 /* log0Buf */
    {PULSE_HIGH | 560, PULSE_LOW | (1690 - 26)},               /* log1Buf */
    PULSE_HIGH | 560,                                  /* stopBuf */
    20                                                  /* tolerance percentage is 10% */
};
...
/**
  ...
  * @retval   none.
  */
static void InvsePulse(IR_DataType *pBuf, uint16_t len)
{
    uint16_t i = 0;
...
{
    return (((counter & IR_DATA_MSK) * 1000) / freq);
}
...
/**
  ...
  * @retval None
  */
IR_RETURN_Type IR_NECEncode(uint8_t freqency, uint8_t address, uint8_t command,
                            IR_DataTypeDef *IR_DataStruct)
{
    IR_DataStruct->carrierFreq = freqency;
    IR_DataStruct->codeLen = 4;
    IR_DataStruct->code[0] = address;
    IR_DataStruct->code[1] = ~address;
    IR_DataStruct->code[2] = command;
...
/**
  ...
  * @retval None
  */
IR_RETURN_Type IR_NECDecode(uint8_t freqency, uint8_t *pAddr, uint8_t *pCmd,
                            IR_DataTypeDef *IR_DataStruct)
{
    ...
    *pAddr = IR_DataStruct->code[0];
    *pCmd = IR_DataStruct->code[2];
