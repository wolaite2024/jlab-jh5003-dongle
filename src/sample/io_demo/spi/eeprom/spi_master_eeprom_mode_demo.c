...
**********************************************************************************************************
* @file     spi_master_eeprom_mode_demo.c
* @brief    This file provides demo code of SPI master with eeprom mode read  data by dma.
* @details  In eeprom read mode the transmit data is used to transmit an opcode and/or an address to the EEPROM
*           device. Typically this takes three data frames (8-bit opcode followed by 8-bit upper address and 8-bit lower
*           address). During the transmission of the opcode and address, no data is captured by the receive logic.
* @author   justin
* @date     2023-01-10
* @version  v1.0
*********************************************************************************************************
...
#define SPI_MASTER_RX_DMA_CHANNEL_NUM    spi_master_eeprom_rx_ch_num
#define SPI_MASTER_RX_DMA_CHANNEL        DMA_CH_BASE(spi_master_eeprom_rx_ch_num)
#define SPI_MASTER_RX_DMA_IRQ            DMA_CH_IRQ(spi_master_eeprom_rx_ch_num)
#define SPI_MASTER_RX_DMA_VECTOR         DMA_CH_VECTOR(spi_master_eeprom_rx_ch_num)
...
static uint8_t spi_master_eeprom_rx_ch_num = 0x55;
...
static void spi_master_read_dma_init(void)
{
    ...
    if (!GDMA_channel_request(&spi_master_eeprom_rx_ch_num, spi_master_rx_dma_handler, true))
    {
        return;
    }
...
static void spi_dma_rx_data(uint8_t *p_recv_buf, uint16_t len)
{
    ...
    /* Send an opcode and/or an address to the EEPROM device, the master
       will continue to send dummy data (0) after the transmission is complete */
    SPI_SendBuffer(SPI_MST, send_buf, 3);
}

void spi_master_eeprom_mode_demo(void)
{
    IO_PRINT_INFO0("spi_master_eeprom_mode_demo");
