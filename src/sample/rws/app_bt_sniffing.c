...
#endif
    [APP_BT_SNIFFING_CONNECTED_TO_AP_WITH_CIS_ADV] = {0x06, 0x02}, //0x0D, LEA CIS adv is enabled
};
...
#if F_APP_ERWS_SUPPORT && F_APP_EXT_MIC_SWITCH_SUPPORT && F_APP_LEA_SUPPORT
    //leaudio connected & no b2s connected
    if (app_link_get_lea_link_num() && app_link_get_b2s_link_num() == 0)
    {
        check_result = true;
    }
#endif
    ...
    if (p_link != NULL && app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
    {
        ...
        else if (p_link->sniffing_type == BT_SNIFFING_TYPE_SCO)
        {
            ...
            if (!p_link->sco_initial)
            {
                ...
                {
                    bt_hfp_audio_connect_cfm(bd_addr, app_multi_check_accept_sco_conn());
                    p_link->pending_ind_confirm = false;
                }
            }
            else
        ...
        (app_db.remote_session_state == REMOTE_SESSION_STATE_CONNECTED))
    {
        ...
        else
        {
            ...
            else if (ble_ext_adv_is_ongoing())
            {
                ...
                {
#if F_APP_GAMING_DONGLE_SUPPORT
                    if (app_device_is_connected_to_ap() == true)
                    {
                        recovery_link_a2dp_idle_slot = htpoll[APP_BT_SNIFFING_CONNECTED_TO_AP_WITH_CIS_ADV].idle_slot;
                        recovery_link_a2dp_idle_skip = htpoll[APP_BT_SNIFFING_CONNECTED_TO_AP_WITH_CIS_ADV].idle_skip;
                    }
                    else
#endif
                    {
