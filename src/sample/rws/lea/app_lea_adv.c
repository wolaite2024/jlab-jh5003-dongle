...
#define APP_LEA_FAST_ADV_TIME   180000

typedef enum
{
    LEA_ADV_TIMER_SLOW_INTERVAL,
} T_APP_LEA_ADV_TIMER;
...
static void app_lea_adv_timeout_cb(uint8_t timer_evt, uint16_t param)
{
    switch (timer_evt)
    {
    case LEA_ADV_TIMER_SLOW_INTERVAL:
        {
...
{
#if F_APP_DISABLE_LEA_PHONE_SUPPORT
    //We do not support LEA phone temporarily.
    APP_PRINT_INFO0("app_lea_adv_start: do not advertise for LEA phone!");
    return ;
#else
    if (app_db.device_state == APP_DEVICE_STATE_ON)
    {
        if (((app_cfg_const.iso_mode & LE_AUDIO_CIS) ||
            ...
            (app_link_is_b2s_link_num_full() == false))
        {
            ...
            app_start_timer(&timer_idx_slow_interval, "slow_interval",
                            app_lea_adv_timer_id, LEA_ADV_TIMER_SLOW_INTERVAL, 0, false,
...
void app_lea_adv_update(void)
{
    ...
    audio_adv_len = app_lea_adv_set_ext_data(audio_adv_flag, service_num);
