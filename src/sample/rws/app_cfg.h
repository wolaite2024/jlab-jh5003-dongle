...
// App base cpu freq
#if F_APP_POWER_TEST && TARGET_RTL8773DO
...
#define ULTRA_LOW_LATENCY_MS_WITH_MULTI_SRC 10
#define ULTRA_LOW_LATENCY_MS_WITH_BLE_ADV   15
#define ULTRA_LOW_LATENCY_MS_WITH_LINKBACK  25

#if F_APP_SUPPORT_SPATIAL_AUDIO && TARGET_RTL8753GFE
...
typedef struct
{
    ...
    uint8_t eq_idx;
    uint8_t audio_effect_normal_type;
    uint8_t audio_effect_gaming_type;
    ...
    union
    {
        ...
        struct
        {
            ...
            uint8_t led_pwm_control_level : 3;
        };
    };
    ...
    union
    {
        uint8_t offset_led_breath_mode_min_brightness_level;

        struct
        {
            uint8_t led_breath_mode_min_brightness_level : 4;
            /* support common adv even legacy gaming is streaming */
            uint8_t legacy_gaming_support_common_adv : 1;
            uint8_t dongle_rf_mode : 2;
            uint8_t led_breath_mode_rsv : 1;
        };
    };

    uint8_t ps_vol_gain;
    uint8_t android_vol_gain;
    uint8_t switch_vol_gain;
} T_APP_CFG_NV;
...
typedef struct
{
    ...
    uint8_t audio_rsv2[21];
    uint8_t dual_audio_normal : 4;
