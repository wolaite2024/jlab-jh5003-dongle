...
#if F_APP_GAMING_DONGLE_SUPPORT
static bool is_connected_to_ap = false;
#endif
    ...
    case BP_EVENT_BUD_REMOTE_CONN_CMPL:
        {
            ...
            if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
            {
                ...
                app_dual_audio_sync_all_app_key_val();
#endif
            }
    ...
    case BP_EVENT_SRC_DISCONN_NORMAL:
        {
            ...
            p_le_link = app_link_find_le_link_by_addr(event_param->bd_addr);
            if (p_le_link)
            {
            ...
            {
                app_teams_ble_policy_handle_br_link_disconn(event_param->bd_addr);
            }
#endif
        }
        break;
...
#if F_APP_GAMING_DONGLE_SUPPORT
static bool app_device_is_ap(uint8_t *addr)
{
    T_APP_AP_INFO ap_info[] =
    {
        {{0x00, 0x07, 0x80}},
    };
    ...
    for (i = 0; i < sizeof(ap_info) / sizeof(T_APP_AP_INFO); i++)
    {
        if (!memcmp(device_oui, ap_info[i].oui, sizeof(ap_info[i].oui)))
        {
            ret = true;
            break;
        }
    ...
    APP_PRINT_TRACE2("app_device_is_ap: ret %d, oui %b", ret, TRACE_BINARY(3, device_oui));
...
static void app_device_bt_cback(T_BT_EVENT event_type, void *event_buf, uint16_t buf_len)
{
    ...
    case BT_EVENT_ACL_CONN_DISCONN:
        {
#if F_APP_GAMING_DONGLE_SUPPORT
            if (app_device_is_ap(param->acl_conn_disconn.bd_addr))
            {
...
#if F_APP_GAMING_DONGLE_SUPPORT
            if (app_device_is_ap(param->remote_name_rsp.bd_addr))
            {
                is_connected_to_ap = true;
            }
#endif
...
#if F_APP_GAMING_DONGLE_SUPPORT
bool app_device_is_connected_to_ap(void)
{
    APP_PRINT_TRACE1("app_device_is_connected_to_ap: %d", is_connected_to_ap);
    return is_connected_to_ap;
}
#endif
