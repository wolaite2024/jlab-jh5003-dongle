...
typedef enum
{
    ENGAGE_ACTION_ADV_SET,
    ENGAGE_ACTION_ADV_START,
    ENGAGE_ACTION_ADV_STOP,
} T_ENGAGE_ACTION;
...
/**  @brief engage lib report adv action to app */
typedef void (*T_ENGAGE_ACTION_FUN)(T_ENGAGE_ACTION action, uint8_t *adv_data, uint16_t adv_len);
...
/**
    * @brief  engage action callback register
    * @param  action: callback function
    * @return void
    */
void engage_action_register_cb(T_ENGAGE_ACTION_FUN action);

/**
    * @brief  handle engage adv by other receive path (ex: via dongle)
    * @param  addr : peer bud addr
    * @param  data : adv data
    * @param  len  : adv len
    * @return void
    */
void engage_adv_report(uint8_t *addr, uint8_t *data, uint16_t len);
