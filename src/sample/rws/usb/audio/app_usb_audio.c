...
{
    return ((int16_t)(vol + UAC1_SPK_VOL_MIN) / 256);
}

static void app_usb_audio_ual_spk_vol_db_set(void *handle, T_UAS_VOL *vol)
{
    int16_t gain = app_usb_spk_vol_convert_to_db(vol->cur);
    uint16_t volume = (int16_t)vol->cur + UAC1_SPK_VOL_MIN;
    usb_spk_vol.uac_spk_vol_gain = gain;

    if ((volume == UAC1_SPK_VOL_MIN) || (volume == UAC1_SPK_VOL_MAX))
    {
        usb_spk_vol.is_max_min_vol = true;
    }
    else
    {
        usb_spk_vol.is_max_min_vol = false;
    }

    app_usb_volume_db_set(handle, gain, CTRL_FROM_HOST);
}

void app_usb_audio_local_gain_change(uint8_t type)
{
    app_usb_audio_volume_control_handle((T_APP_VOL_CTRL)type, app_usb_audio_db.playback->handle);
}

static void app_usb_host_type_sync_cback(T_OS_TYPE type)
{
    app_usb_volume_db_set(app_usb_audio_db.playback->handle, usb_spk_vol.uac_spk_vol_gain,
                          CTRL_FROM_HOST);
}
#endif
...
#if F_APP_GAMING_DONGLE_TRANS_UAC_VOL_TO_HEADSET
            app_usb_audio_ual_spk_vol_db_set(handle, vol);
#else
...
static bool app_usb_audio_ual_mic_release(void *handle)
{
    ...
    if (handle)
    {
        ...
        {
            app_nrec_detach(handle, audio->nrec_instance);
...
#if F_APP_GAMING_DONGLE_TRANS_UAC_VOL_TO_HEADSET
                    app_usb_volume_db_set(audio->handle, usb_spk_vol.uac_spk_vol_gain, CTRL_FROM_HOST);
#endif
                }
                break;
