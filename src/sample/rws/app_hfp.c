...
typedef enum
{
    ...
    APP_HFP_TIMER_PLAY_INCOMING_TONE = 0x08,
} T_APP_HFP_TIMER;
...
#if F_APP_EXT_MIC_SWITCH_SUPPORT
static bool origin_voice_secondary_mic_enable = false;
#endif
...
void app_hfp_voice_nr_enable(void)
{
    ...
    enable = (app_hfp_get_call_status() >= APP_CALL_ACTIVE) ? 1 : 0;
    ...
    case AUDIO_EVENT_TRACK_STATE_CHANGED:
        {
            ...
            if (origin_voice_secondary_mic_enable)
            {
                if (app_db.ext_mic_plugged)
                {
                    sec_mic_enable = false;
                }
                else
                {
                    sec_mic_enable = true;
                ...
                flash_dsp_set_voice_sec_mic_cfg(sec_mic_enable);
                flash_dsp_set_record_sec_mic_cfg(sec_mic_enable);
            }
#endif
        }
        break;
...
{
#if F_APP_EXT_MIC_SWITCH_SUPPORT
    origin_voice_secondary_mic_enable = flash_dsp_get_voice_sec_mic_cfg();
#endif
...
void app_hfp_mute_ctrl(void)
{
        ...
        (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_SINGLE))
    {
        if (hf_active)
        {
            ...
            if (app_audio_is_mic_mute())
            {
#if CONFIG_REALTEK_APP_TEAMS_FEATURE_SUPPORT || F_APP_CONFERENCE_DONGLE_SUPPORT
