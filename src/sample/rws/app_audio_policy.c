...
#define HANDLE_LIST_TRACK_NUM_MAX   (HANDLE_LIST_VOICE_TRACK_NUM_MAX + HANDLE_LIST_RECORD_TRACK_NUM_MAX)
...
#endif
static bool audio_tone_type_play(T_APP_AUDIO_TONE_TYPE tone_type, bool flush, bool relay,
    ...
    case AUDIO_EVENT_TRACK_DATA_IND:
        {
...
#endif
            {
...
#endif
                {
                    ...
                    if (param->track_data_ind.handle != p_link->sco_track_handle)
                    {
                        break;
                    }
...
static void app_audio_save_a2dp_config(uint8_t *a2dp_cfg, T_AUDIO_FORMAT_INFO *format_info)
{
    ...
    else if (cfg->codec_type == BT_A2DP_CODEC_TYPE_LC3)
    {
        ...
        case BT_A2DP_LC3_CHANNEL_NUM_2:
            format_info->attr.lc3.chann_location = AUDIO_CHANNEL_LOCATION_FL |
...
static void app_audio_a2dp_stream_start_handle(uint8_t *bd_addr, uint8_t *audio_cfg)
{
    ...
    if (p_link->a2dp_track_handle != NULL)
    {
        ...
        else
        {
            ...
            {
#if F_APP_MUTLILINK_SOURCE_PRIORITY_UI
                app_multilink_customer_music_event_handle(active_a2dp_idx, JUDGE_EVENT_A2DP_START);
#else
#if F_APP_LINEIN_SUPPORT
                if (app_cfg_const.line_in_support)
                {
...
static uint16_t app_audio_relay_cback(uint8_t *buf, uint8_t msg_type, bool total)
{
    ...
    case APP_REMOTE_MSG_SYNC_BOOM_MIC_IN:
        {
            payload_len = 1;
            msg_ptr = (uint8_t *)&app_db.local_boom_mic_in;
            skip = false;
        }
        break;
#endif
                                  ...
                                  T_REMOTE_RELAY_STATUS status)
{
    ...
    case APP_REMOTE_MSG_VOICE_NR_SWITCH:
        {
            if ((status == REMOTE_RELAY_STATUS_SYNC_SENT_OUT) ||
    ...
    case APP_REMOTE_MSG_SYNC_BOOM_MIC_IN:
        {
            ...
            {
                uint8_t *p_info = (uint8_t *)buf;
                app_db.remote_boom_mic_in = (bool)p_info[0];

                if ((app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY) && (app_db.remote_boom_mic_in == true))
                {
                    app_roleswap_ctrl_check(APP_ROLESWAP_CTRL_EVENT_BOOM_MIC_IN);
                }
            }
        }
        break;
...
#if F_APP_CUSTOMIZED_SBC_VP_FLOW_SUPPORT
bool app_audio_tone_type_play_with_addr(T_APP_AUDIO_TONE_TYPE tone_type, bool flush, bool relay,
                                        uint32_t addr, uint32_t len)
{
    bool ret = false;

    ret = audio_tone_type_play(tone_type, flush, relay, addr, len);
                                 ...
                                 uint32_t addr, uint32_t len)
{
    ...
    APP_PRINT_TRACE3("app_audio_tone_type_play: customized = %d, addr = 0x%x, len = 0x%x",
                     use_customized_vp,
                     vp_address,
                     vp_len);
