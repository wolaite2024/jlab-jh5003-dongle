...
bool app_multi_check_accept_sco_conn(void)
{
    bool accept_sco_conn = true;
    uint8_t cause = 0;
...
exit:
    ...
    if (accept_sco_conn == false)
    {
        APP_PRINT_TRACE1("app_multi_check_accept_sco_conn: reject sco conn with cause -%d", cause);
    }

    return accept_sco_conn;
}
                    ...
                    if (app_cfg_const.enable_multi_sco_disc_resume)
                    {
                        ...
                        else
                        {
                            ...
                            if (!app_bt_sniffing_start(param->sco_conn_ind.bd_addr, BT_SNIFFING_TYPE_SCO))
                            {
                                ...
                                if (p_link != NULL)
                                {
                                    bt_hfp_audio_connect_cfm(param->sco_conn_ind.bd_addr, app_multi_check_accept_sco_conn());
                                }
                            }
                        }
                    }
