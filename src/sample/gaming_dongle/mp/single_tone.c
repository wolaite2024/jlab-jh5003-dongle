...
#define APP_IO_TIMER_DUT_TONE_REPEAT                        0
#define APP_IO_TIMER_DUT_MODE_AUTO_POWER_OFF                1
...
static void dut_hci_cmd_reset(void)
{
    ...
    uint8_t *p_buf = os_mem_alloc(RAM_TYPE_DATA_ON, cmd_len);
    uint8_t *p = p_buf;
...
static void dut_hci_cmd_write_local_name(uint8_t *p_name, uint8_t name_len)
{
    ...
    uint8_t cmd_len = param_len + 4;
    uint8_t *p_buf = os_mem_zalloc(RAM_TYPE_DATA_ON, cmd_len);
    uint8_t *p = p_buf;
                    ...
                    case MP_HCI_TEST_DUT_MODE:
                        {
                            ...
                            if (app_dlps_is_support_external_pa())
                            {
                                ...
                                uint8_t *p_buf = (uint8_t *) malloc(sizeof(uint8_t) * 7);

                                APP_PRINT_TRACE5("mp_hci_handle_msg: before init 0x40058030 = 0x%x, 0x400002A8 = 0x%x, aon_1_0 = 0x%x, aon_1_1 = 0x%x, pinmux = 0x%x",
                                ...
                                *((volatile uint32_t *)0x400002A8) |= BIT(16);
                                ...
                                APP_PRINT_TRACE5("mp_hci_handle_msg: after init 0x40058030 = 0x%x, 0x400002A8 = 0x%x, aon_1_0 = 0x%x, aon_1_1 = 0x%x, pinmux = 0x%x",
                                                 *((volatile uint32_t *)0x40058030), *((volatile uint32_t *)0x400002A8), \
                                                 aon_1_0, aon_1_1, PINMUX0->CFG[2]);
                            }
#endif
...
//            app_auto_power_off_enable(AUTO_POWER_OFF_MASK_POWER_ON, app_cfg_const.timer_auto_power_off);
            ...
            if (app_cfg_const.enable_repeat_dut_test_tone)
            {
                ...
                app_start_timer(&timer_idx_dut_tone_repeat, "dut_tone_repeat",
                                dut_timer_id, APP_IO_TIMER_DUT_TONE_REPEAT, 0, false,
                                5000);
            }
            else
            {
...
static void dut_timeout_cb(uint8_t timer_evt, uint16_t param)
{
    switch (timer_evt)
    {
    case APP_IO_TIMER_DUT_TONE_REPEAT:
        {
            if (timer_idx_dut_tone_repeat != NULL)
            {
                app_stop_timer(&timer_idx_dut_tone_repeat);
//                app_audio_tone_type_play(TONE_DUT_TEST, false, false);
                app_start_timer(&timer_idx_dut_tone_repeat, "dut_tone_repeat",
                                dut_timer_id, APP_IO_TIMER_DUT_TONE_REPEAT, 0, false,
                                5000);
...
void mp_hci_test_init(T_MP_HCI_TEST_ITEM mp_hci_test_item)
{
    ...
    case MP_HCI_TEST_DUT_MODE:
        {
            APP_PRINT_INFO0("mp_hci_test_init 0");
            dut_test_init();
        }
        break;
    ...
    case MP_HCI_TEST_SINGLE_TONE:
        {
            APP_PRINT_INFO0("mp_hci_test_init 1");
