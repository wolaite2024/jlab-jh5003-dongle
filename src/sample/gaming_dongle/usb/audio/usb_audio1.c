...
typedef struct _usb_audio_freq_attr
{
    T_USB_AUDIO_DIR dir;
    uint8_t         index;
    uint32_t        freq;
} T_USB_AUDIO_FREQ_ATTR;
...
#define UAC1_ISO_OUT_ENDPOINT_ADDR      0x03
#define UAC1_ISO_IN_ENDPOINT_ADDR       0x84
#define ISO_OUT_FEEDBACK_ENDPOINT_ADDR  0x83
#else
#define UAC1_ISO_OUT_ENDPOINT_ADDR      0x03
#define UAC1_ISO_IN_ENDPOINT_ADDR       0x83
#define ISO_OUT_FEEDBACK_ENDPOINT_ADDR  0x00
#endif
...
static const T_UAC1_OT_DESC     output_terminal_desc0 =
{
    ...
    .wTerminalType      = UAC1_OUTPUT_TERMINAL_HEADPHONES, // SWITCH require it to be 0x302
    .bAssocTerminal     = 0,
...
static const T_UAC1_IT_DESC     input_terminal_desc1 =
{
    ...
    .wTerminalType      = UAC1_BI_DIRECTIONAL_TERMINAL_HEADSET,
    .bAssocTerminal     = 0,
...
static T_UAC1_FMT_TYPE_I_DESC(spk, UAC1_SPK_SAM_RATE_NUM) format_type_i_desc1 =
{
...
static T_UAC1_STD_ISO_EP_DESC     out_ep_desc_hs =
{
    ...
    .bEndpointAddress   = UAC1_ISO_OUT_ENDPOINT_ADDR,
#if F_APP_USB_AUDIO_FEEDBACK_SUPPORT
    .bmAttributes       = UAC1_EP_XFER_ISOC | UAC1_EP_SYNC_ASYNC,
#else
    .bmAttributes       = UAC1_EP_XFER_ISOC | UAC1_EP_SYNC_ADAPTIVE,
#endif
    .wMaxPacketSize     = UAC_SPK1_MAX_PACKET_SIZE,
    .bInterval          = 4,
    .bRefresh           = 0,
    .bSynchAddress      = ISO_OUT_FEEDBACK_ENDPOINT_ADDR,
};
...
#if F_APP_USB_AUDIO_FEEDBACK_SUPPORT
static T_UAC1_STD_ISO_EP_DESC     iso_out_feedback_ep_desc_hs =
{
    .bLength            = sizeof(T_UAC1_STD_ISO_EP_DESC),
    .bDescriptorType    = UAC1_DT_ENDPOINT,
    .bEndpointAddress   = ISO_OUT_FEEDBACK_ENDPOINT_ADDR,
    .bmAttributes       = UAC1_EP_XFER_ISOC | UAC1_EP_USAGE_FEEDBACK,
    .wMaxPacketSize     = 4,
    .bInterval          = 4,
    .bRefresh           = 3,
    .bSynchAddress      = 0,
...
static T_UAC1_STD_ISO_EP_DESC     iso_out_feedback_ep_desc_fs =
{
    .bLength            = sizeof(T_UAC1_STD_ISO_EP_DESC),
    .bDescriptorType    = UAC1_DT_ENDPOINT,
    .bEndpointAddress   = ISO_OUT_FEEDBACK_ENDPOINT_ADDR,
...
static T_UAC1_FMT_TYPE_I_DESC(mic, UAC1_MIC_SAM_RATE_NUM) format_type_i_desc2 =
{
...
#if TARGET_RTL8763EAU | TARGET_RTL8773DO
    .tSamFreq           =
    {
        [0] = {[0] = 0x80, [1] = 0xBB, [2] = 0x00},
    },
...
static const T_UAC1_STD_ISO_EP_DESC     in_ep_desc_hs =
{
    .bLength            = sizeof(T_UAC1_STD_ISO_EP_DESC),
    .bDescriptorType    = UAC1_DT_ENDPOINT,
    .bEndpointAddress   = UAC1_ISO_IN_ENDPOINT_ADDR,
    .bmAttributes       = UAC1_EP_XFER_ISOC | UAC1_EP_SYNC_ASYNC,
#if TARGET_RTL8763EAU | TARGET_RTL8773DO
    .wMaxPacketSize     = (UAC1_MIC_SAM_RATE_MAX / 1000) * (UAC1_MIC_BIT_RES / 8) * UAC1_MIC_CHAN_NUM,
#else
    .wMaxPacketSize     = 32 * (UAC1_MIC_BIT_RES / 8) * UAC1_MIC_CHAN_NUM,
#endif
    .bInterval          = 4,
    .bRefresh           = 0,
    .bSynchAddress      = 0,
...
#if F_APP_USB_AUDIO_FEEDBACK_SUPPORT
    (void *) &iso_out_feedback_ep_desc_hs,
#endif
    NULL
};
...
static const void  *uac1_descs_spk_fs[] =
{
    ...
    (void *) &iso_out_ep_desc,
#if F_APP_USB_AUDIO_FEEDBACK_SUPPORT
    (void *) &iso_out_feedback_ep_desc_fs,
#endif
    NULL
};
#endif
...
#if UAC_SPK_SUPPORT
static const T_USB_AUDIO_DRIVER_CTRL_ENTITY(UAC1_ID_FEATURE_UNIT1,
                                            UAC1_SPK_UNIT_CTRL_NUM) uac1_ctrl_entity0 =
{
    .entity_id = UAC1_ID_FEATURE_UNIT1,
    .ctrl_num = UAC1_SPK_UNIT_CTRL_NUM,
...
#if UAC_MIC_SUPPORT
static const T_USB_AUDIO_DRIVER_CTRL_ENTITY(UAC1_ID_FEATURE_UNIT2,
                                            UAC1_MIC_UNIT_CTRL_NUM) uac1_ctrl_entity1 =
{
    .entity_id = UAC1_ID_FEATURE_UNIT2,
    .ctrl_num = UAC1_MIC_UNIT_CTRL_NUM,
...
#if F_APP_USB_AUDIO_FEEDBACK_SUPPORT
RAM_TEXT_SECTION
static int usb_audio_feedback_d(uint8_t *buf, uint16_t len)
{
    USB_AUDIO1_PIPE_CALLBACK(usb_audio1_pipe->feedback_d, usb_audio1_pipe, buf, len);

    *(uint32_t *)buf = TO_FB_FORMAT_10_14(*(uint32_t *)buf);
    return 0;
}
#endif
...
#if TARGET_RTL8763EAU | TARGET_RTL8773DO
static uint8_t usb_audio_reconfigure_freq(T_USB_AUDIO_FREQ_ATTR freq_attr, uint8_t uac_ver)
{
    APP_PRINT_TRACE2("usb_audio_reconfigure_freq, index:%d, freq:%d", freq_attr.index, freq_attr.freq);
    if (freq_attr.dir == USB_AUDIO_DIR_OUT)
    {
#if UAC_SPK_SUPPORT
        if (freq_attr.index < UAC1_SPK_SAM_RATE_NUM)
        {
            memcpy(&format_type_i_desc1.tSamFreq[freq_attr.index], &freq_attr.freq, 3);
        }
        else
        {
            return 0xFF;
    ...
    {
#if UAC_MIC_SUPPORT
        if (freq_attr.index < UAC1_MIC_SAM_RATE_NUM)
        {
            memcpy(&format_type_i_desc2.tSamFreq[freq_attr.index], &freq_attr.freq, 3);
        }
        else
        {
            return 0xFF;
...
static uint16_t usb_audio_get_us_sample_rate(void)
{
    uint16_t us_sample_rate = USB_AUDIO_US_SAMPLE_RATE;

#if F_APP_USB_GIP_SUPPORT
    if (app_gip_get_switch_mode())
    {
        us_sample_rate = UAC_SAM_RATE_16000;
    }
#endif

    return us_sample_rate;
}
...
#if TARGET_RTL8763EAU | TARGET_RTL8773DO
    T_USB_AUDIO_FREQ_ATTR freq_attr;
#if UAC_SPK_SUPPORT
    freq_attr.dir = USB_AUDIO_DIR_OUT;
#if F_APP_USB_AUDIO_96K_24BIT
    freq_attr.freq = 0x17700;
#else
    freq_attr.freq = (USB_AUDIO_DS_SAMPLE_RATE == UAC_SAM_RATE_48000) ? 0xBB80 : 0x3E80;
#endif
    freq_attr.index = 0;
    usb_audio_reconfigure_freq(freq_attr, USB_AUDIO_VERSION_1);
#endif
#if UAC_MIC_SUPPORT
    freq_attr.dir = USB_AUDIO_DIR_IN;
    freq_attr.freq = (usb_audio_get_us_sample_rate() == UAC_SAM_RATE_48000) ? 0xBB80 : 0x3E80;
    freq_attr.index = 0;
    usb_audio_reconfigure_freq(freq_attr, USB_AUDIO_VERSION_1);
#endif
#endif
...
#if F_APP_USB_AUDIO_FEEDBACK_SUPPORT
    cbs.feedback_d = usb_audio_feedback_d;
#else
    cbs.feedback_d = NULL;
#endif
