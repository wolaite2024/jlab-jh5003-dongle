 ...
 * usb_hid.h
 *
 * \brief   USB HID interface number and endpoint number of consumer control report.
 *          \ref intf_num: Interface number.
 *          \ref ep_num: Endpoint number.
 *
 * \ingroup USB_HID
 */
typedef struct _t_hid_consumer_control_intf_info
{
    uint8_t intf_num;
    uint8_t ep_num;
} T_HID_CONSUMER_CONTROL_INTF_INFO;
...
/**
T_HID_CONSUMER_CONTROL_INTF_INFO usb_hid_consumer_control_intf_info_peek(void);
 * usb_hid.h
 *
 * \brief   usb hid init
 *
