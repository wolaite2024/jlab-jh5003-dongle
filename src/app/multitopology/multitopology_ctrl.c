...
typedef enum
{
    MTC_DEV_STATE_OFF       = 0x00,
    MTC_DEV_STATE_ON        = 0x01,
    MTC_DEV_STATE_OFF_ING   = 0x02,
} T_MTC_DEV_STATE;
...
typedef enum
{
    ...
    MTC_IF_TO_AP_RESUME_A2DP                        = MTC_IF_GROUP_AUDIO_POLICY | 0x01,
    MTC_IF_TO_AP_STOP_A2DP                          = MTC_IF_GROUP_AUDIO_POLICY | 0x02,
    MTC_IF_FROM_AP_SCO_CMPL                         = MTC_IF_GROUP_AUDIO_POLICY | 0x80,
    MTC_IF_FROM_AP_SNIFFING_CHECK                   = MTC_IF_GROUP_AUDIO_POLICY | 0x81,
    MTC_IF_FROM_AP_SHUTDOWN                         = MTC_IF_GROUP_AUDIO_POLICY | 0x82,

    MTC_IF_TO_ML_LEA_START                          = MTC_IF_GROUP_MULTILINK | 0x00,
    MTC_IF_TO_ML_LEA_STOP                           = MTC_IF_GROUP_MULTILINK | 0x01,
    MTC_IF_FROM_ML_CH_SNIFFING                      = MTC_IF_GROUP_MULTILINK | 0x80,
    MTC_IF_FROM_ML_CH_LEA_CALL                      = MTC_IF_GROUP_MULTILINK | 0x81,

    MTC_IF_TO_LCIS_LEA_TRACK                        = MTC_IF_GROUP_LEA_CIS | 0x00,
    MTC_IF_TO_LCIS_LEA_PACS_CHANGE                  = MTC_IF_GROUP_LEA_CIS | 0x01,
    MTC_IF_FROM_LCIS_ASCS_CP_ENABLE                 = MTC_IF_GROUP_LEA_CIS | 0x80,
    MTC_IF_FROM_LCIS_TER_CIS                        = MTC_IF_GROUP_LEA_CIS | 0x81,

    MTC_IF_TO_LBIS                                  = MTC_IF_GROUP_LEA_BIS | 0x00,
    MTC_IF_FROM_LBIS_CHECK_RESUME                   = MTC_IF_GROUP_LEA_BIS | 0x80,
} T_MTC_MSG;
/*============================================================================*
 *                              Variables
 *============================================================================*/
...
{
    APP_PRINT_TRACE1("mtc_set_dev: para %d", para);
    mtc_db.device.dev_state = (T_MTC_DEV_STATE)para;
}
...
{
    if (app_lea_mgr_ase_exist_conn(app_lea_mcp_get_active_conn_handle()))
    {
        app_lea_uca_link_sm(app_lea_mcp_get_active_conn_handle(), LEA_PAUSE_LOCAL, NULL);
        return true;
    }
    return false;
}
...
static void mtc_terminate_bis(void)
{
    ...
    else
    {
        ...
        mtc_if_to_ml((T_MTC_IF_MSG)MTC_IF_TO_ML_LEA_STOP, NULL, NULL);
    }
}
...
void mtc_resume_bis(uint8_t para)
{
    ...
    {
        app_mmi_handle_action(MMI_BIG_START);
...
void mtc_check_lea_track(void)
{
    ...
    {
        mtc_if_to_lcis((T_MTC_IF_MSG)MTC_IF_TO_LCIS_LEA_TRACK, NULL, NULL);
    }
}
...
bool mtc_stream_pause(bool param_update)
{
    ...
    if (app_db.remote_session_state == REMOTE_SESSION_STATE_CONNECTED)
    {
        if (app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_PRIMARY)
        {
            ...
            app_link_disallow_legacy_stream(true);
            mtc_a2dp_release();
            mtc_if_to_ap((T_MTC_IF_MSG)MTC_IF_TO_AP_STOP_A2DP, p_link->bd_addr, NULL);
#endif
            stop_a2dp = true;
        }
    }
...
{
    mtc_if_to_lcis((T_MTC_IF_MSG)MTC_IF_TO_LCIS_LEA_PACS_CHANGE, &enable, NULL);
}
...
static void mtc_cis_ase_release(T_MTC_STREAM_TYPE type)
{
    ...
    else
    {
        ...
        {
            mtc_ase_release();
            mtc_if_to_ap((T_MTC_IF_MSG)MTC_IF_TO_AP_RESUME_A2DP, &a2dp_para, NULL);
...
bool mtc_sync_active_mode(void)
{
    ...
    app_lea_mgr_get_link_info(&buf[48], &buf[49], &buf[50]);
...
bool mtc_topology_dm(uint8_t event)
{
    ...
    case MTC_TOPO_EVENT_CCP_CALL:
        {
            if (app_lea_ccp_get_call_status() == APP_CALL_IDLE)
            {
                ...
                if (app_hfp_get_call_status() != APP_CALL_IDLE)
                {
                    ...
                    {
                        mtc_if_to_ap((T_MTC_IF_MSG)MTC_IF_TO_AP_RESUME_SCO, test_bd_addr, NULL);
    ...
    case MTC_TOPO_EVENT_BR_ROLE_SWAP:
        {
            ...
            else  if (prio_mode == MTC_LINK_ACT_LAST_ONE)
            {
                ...
                if (app_cfg_const.enable_24g_bt_audio_source_switch)
                {
                    ...
                    if (app_cfg_nv.allowed_source == ALLOWED_SOURCE_BT)
                    {
                        if (app_dongle_get_connected_lea_phone_link())
                        {
                            edr_prio_high = false;
                            mtc_set_btmode(MULTI_PRO_BT_CIS);
                            disallow_sniffing = true;
    ...
    if ((mtc_db.device.cur_dev_active_mode != bt_mode || pre_bt_mode != bt_mode ||
        ...
        mtc_db.device.dev_state == MTC_DEV_STATE_ON)
    {
                     ...
                     uint16_t len)
{
    ...
    case MTC_REMOTE_SYNC_MTC_STATE:
        {
                ...
                status == REMOTE_RELAY_STATUS_ASYNC_SENT_OUT)
            {
                ...
                if ((app_cfg_nv.bud_role == REMOTE_SESSION_ROLE_SECONDARY) &&
                    (active_mode & (MTC_MODE_CIS_PLAYBACK | MTC_MODE_CIS_VOICE)))
                {
                    app_lea_uca_set_ready_to_downstream(true);
                }
            }
    ...
    case MTC_REMOTE_SYNC_LEA_REL_A2DP:
        {
            mtc_if_to_ap((T_MTC_IF_MSG)MTC_IF_TO_AP_STOP_A2DP, a2dp_para.bd_addr, NULL);
        }
        break;
...
T_MTC_RESULT mtc_if_fm_ap_handle(T_MTC_MSG msg, void *inbuf, void *outbuf)
{
    ...
    case MTC_IF_FROM_AP_SNIFFING_CHECK:
        {
...
T_MTC_RESULT mtc_if_fm_ml_handle(T_MTC_MSG msg, void *inbuf, void *outbuf)
{
    T_MTC_RESULT mtc_result = MTC_RESULT_SUCCESS;
    //APP_PRINT_INFO1("mtc_if_fm_ml_handle: msg %x", msg);
    switch (msg)
    {
    case MTC_IF_FROM_ML_CH_SNIFFING:
        {
            if (mtc_get_sniffing())
            {
                *(uint8_t *)outbuf = true;
    ...
    case MTC_IF_FROM_ML_CH_LEA_CALL:
        {
            if (app_lea_ccp_get_call_status() != APP_CALL_IDLE)
            {
                *((uint8_t *)outbuf) = true;
...
{
    T_MTC_RESULT mtc_result = MTC_RESULT_SUCCESS;
    //APP_PRINT_INFO1("mtc_if_fm_lcis_handle: msg %x", msg);
    switch (msg)
    {
    case MTC_IF_FROM_LCIS_ASCS_CP_ENABLE:
        {
#if F_APP_GAMING_DONGLE_SUPPORT
...
{
    T_MTC_RESULT mtc_result = MTC_RESULT_SUCCESS;
    //APP_PRINT_INFO1("mtc_if_fm_lbis_handle: msg %x", msg);
    switch (msg)
    {
    case MTC_IF_FROM_LBIS_CHECK_RESUME:
        {
